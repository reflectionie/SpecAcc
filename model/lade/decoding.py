...
def sample_proxy(self, *args, **kwargs):
    ...
    USE_LADE = True if config["speculative_method"] == "lookahead" else False
    # USE_LADE = int(os.environ.get("USE_LADE", 0))
...
def update_token_map(token_map, lst_token, past_tokens, new_results, LEVEL, WINDOW_SIZE, GUESS_SET_SIZE):
    if GUESS_SET_SIZE != -1:  # limited guess set size for each key, lru policy
        if lst_token not in token_map:
            token_map[lst_token] = []
        tup = tuple(past_tokens[ll][0]
                    for ll in range(1, LEVEL - 1)) + (new_results[0],)
        ...
        for i in range(1, WINDOW_SIZE):
            ...
            tup = tuple(past_tokens[ll][i] for ll in range(
                1, LEVEL - 1)) + (new_results[i],)
            ...
            if tup in token_map[past_tokens[0][i - 1]]:
                ...
                token_map[past_tokens[0][i - 1]
                          ] = token_map[past_tokens[0][i - 1]][1:] + [tup]
...
def jacobi_sample_multilevel(
    ...
    **model_kwargs,
    ...
    past_tokens = [[set_token() for _ in range(WINDOW_SIZE + LEVEL - 3)]
                   ] + [None for _ in range(LEVEL - 2)]

    if DIST_WORKERS > 1:
        # keep past_tokens always the same on different GPUs
        dist.broadcast_object_list(past_tokens, src=0)
    ...
    while True:
        ...
        if past_tokens[1] is None:
            ...
            past_tokens[1] = torch.argmax(
                outputs.inp_logits, dim=-1)[0].tolist()  # fill window with argmax
            ...
            past_tokens[fill_level +
                        1] = torch.argmax(outputs.inp_logits, dim=-1)[0].tolist()[1:]

            fill_level += 1
        else:

            if guess_tokens is not None:
                probs_next = torch.nn.functional.softmax(
                    next_token_scores, dim=-1)[0]
                hits = []
                # = original model output
                guess_logits = logits_warper(
                    input_ids, outputs.guess_logits[0])
                guess_probs = torch.nn.functional.softmax(guess_logits, dim=-1)
                # guess_results = torch.argmax(outputs.guess_logits, dim=-1)[0].tolist()
                guess_indices = list(
                    range(outputs.guess_logits.size(1) // GUESS_SIZE))
                ...
                for idx_in_ngram in range(GUESS_SIZE):
                    ...
                    while g_idx < len(guess_indices):
                        ...
                        prob_accept = min(1, probs_next[draft_guess].item())
                    ...
                    if is_accept:
                        ...
                        new_token_gen = torch.multinomial(
                            probs_next, num_samples=1).item()
                ...
                next_tokens = torch.multinomial(
                    probs_next, num_samples=1).squeeze(1)
                hits = [next_tokens.item()]
            ...
            assert len(
                past_tokens[LEVEL - 2]) == WINDOW_SIZE and len(new_results) == WINDOW_SIZE

            update_token_map(token_map, lst_token, past_tokens,
                             new_results, LEVEL, WINDOW_SIZE, GUESS_SET_SIZE)
            ...
            if max_hit > 0:
                if not ALWAYS_FWD_ONE:
                    for level in range(LEVEL - 1):
                        past_tokens[level] = past_tokens[level] + \
                            [set_token() for _ in range(max_hit)]

                attention_mask = model_kwargs["attention_mask"]
                model_kwargs["attention_mask"] = torch.cat((attention_mask, torch.ones(
                    1, max_hit, device=attention_mask.device, dtype=attention_mask.dtype)), dim=1)

            if eos_token_id is not None:
                # filter <EOS> (we find too many <EOS> in window lead to numerical error)
                filter_window(past_tokens[LEVEL - 2],
                              eos_token_id[0], set_token)
            ...
            if eos_token_id is not None and hits[hit_ids] == eos_token_id[0]:
                ...
                if POOL_FROM_PROMPT:
                    append_new_generated_pool(
                        all_old_tokens[-LEVEL:], token_map, LEVEL, GUESS_SET_SIZE)
            ...
            if COLOR_PRINT:
                ...
                if max_hit > 1:
                    not_hit = self.tokenizer.decode(all_old_tokens[:-max_hit + 1], skip_special_tokens=True,
                                                    spaces_between_special_tokens=False, clean_up_tokenization_spaces=True,)
                    pt = colored(not_hit[prev:], "blue") + \
                        colored(all_str[len(not_hit):], "blue")
                else:
                    pt = all_str[prev:]
        ...
        input_ids = torch.cat([input_ids, torch.tensor(
            hits[:max_hit + 1], device=input_ids.device, dtype=input_ids.dtype).unsqueeze(0)], dim=-1)
    ...
    if DEBUG and LOCAL_RANK == 0:
        ...
        print("Generated tokens: ", len(all_old_tokens) - init_len, "Total steps: ", steps,
              " Compression ratio: ", round((len(all_old_tokens) - init_len) / steps, 2))
        print("======================================================================================", end="")
        CONFIG_MAP["log"].append([len(all_old_tokens) - init_len,
                                 steps, round((len(all_old_tokens) - init_len) / steps, 2)])
    ...
    while True:
        ...
        if past_tokens[LEVEL - 2] is not None and lst_token in token_map and GUESS_SET_SIZE > 0:
            ...
            if DIST_WORKERS > 1 and guess_tokens is not None:
                ...
                cnt_guess = (len(guess_tokens) // GUESS_SIZE +
                             DIST_WORKERS - 1) // DIST_WORKERS
                guess_base = cnt_guess * LOCAL_RANK
                guess_end = cnt_guess * (LOCAL_RANK + 1)
                guess_tokens = guess_tokens[GUESS_SIZE *
                                            guess_base: GUESS_SIZE * guess_end]
        ...
        if DIST_WORKERS > 1:
            ...
            for l in range(1, len(past_tokens)):
                ...
                past_tokens_inp.append(
                    tokens[window_start: window_end] if tokens is not None else None)
        else:
        ...
        if past_tokens[1] is None:  # filling multi-level window, the very first step is different
            ...
            past_tokens[0] = past_tokens[0][1:]
            past_tokens[1] = torch.argmax(
                outputs.inp_logits, dim=-1)[0].tolist()

            if DIST_WORKERS > 1:
                nn_past_tokens = [copy.deepcopy(past_tokens[1])]
                torch.distributed.broadcast_object_list(
                    nn_past_tokens, src=DIST_WORKERS - 1)
            ...
            for level in range(fill_level + 1):
                past_tokens[level] = past_tokens[level][1:]
            current_past_tokens = torch.argmax(
                outputs.inp_logits, dim=-1)[0].tolist()

            if DIST_WORKERS > 1:
                nn_past_tokens = [None] * DIST_WORKERS
                torch.distributed.all_gather_object(
                    nn_past_tokens, current_past_tokens)
            ...
            # match guess tokens
            if guess_tokens is not None:
                guess_results = torch.argmax(
                    outputs.guess_logits, dim=-1)[0].tolist()
                for eg in range(len(guess_results) // GUESS_SIZE):
                    egx = eg * GUESS_SIZE
                    correct = [first_guess] + \
                        guess_results[egx:egx + GUESS_SIZE]
        ...
        if DIST_WORKERS > 1 and max_hit > 0:
            ...
            for idx, kv in enumerate(outputs.past_key_values):
                past_key_values.append(
                    (kv[0][:, :, :outputs.kvcache_len, :], kv[1][:, :, :outputs.kvcache_len, :]))
            outputs.past_key_values = past_key_values
        else:
            guess_skip_dist = 0
            offset_kv_cache = outputs.step_len - \
                len(guess_tokens)+max_hit_idx * \
                GUESS_SIZE if max_hit > 0 else 0
            for idx, kv in enumerate(outputs.past_key_values):
                # update kv-cache from verification branch
                if max_hit > 0:
                    kv[0][:, :, outputs.kvcache_len:outputs.kvcache_len+max_hit,
                          :] = kv[0][:, :, offset_kv_cache:offset_kv_cache+max_hit, :]
                    kv[1][:, :, outputs.kvcache_len:outputs.kvcache_len+max_hit,
                          :] = kv[1][:, :, offset_kv_cache:offset_kv_cache+max_hit, :]
                past_key_values.append(
                    (kv[0][:, :, :outputs.kvcache_len + max_hit, :], kv[1][:, :, :outputs.kvcache_len + max_hit, :]))
            outputs.past_key_values = past_key_values
        ...
        input_ids = torch.cat([input_ids, torch.tensor(
            hits[:max_hit + 1], device=next_tokens.device, dtype=next_tokens.dtype).unsqueeze(0)], dim=-1)
...
def greedy_search_chat(
    ...
    **model_kwargs,
    ...
    if DEBUG:
        ...
        print("\n==========================ACCELERATION===SUMMARY======================================")
        print("Generated tokens: ", len(all_old_tokens) - init_len,
              "Total steps: ", steps, " Compression ratio: N/A ")
        print("======================================================================================", end="")
