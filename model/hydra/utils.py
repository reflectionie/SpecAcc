...
def generate_hydra_buffers(hydra_choices, device="cuda"):
    ...
    for i in range(len(depth_counts)):
        for j in range(depth_counts[i]):
            ...
            for c in range(len(cur_hydra_choice) - 1):
                ancestor_idx.append(sorted_hydra_choices.index(
                    cur_hydra_choice[:c+1]) + 1)
    ...
    for i in range(len(depth_counts)):
        for j in range(depth_counts[i]):
            cur_hydra_choice = sorted_hydra_choices[start + j]
            hydra_tree_indices[start + j +
                               1] = cur_hydra_choice[-1] + TOPK * i + 1
        start += depth_counts[i]
        ...
        if cur_hydra_choice in retrieve_paths:
            ...
            for c in range(len(cur_hydra_choice)):
                retrieve_indice.append(
                    sorted_hydra_choices.index(cur_hydra_choice[:c+1]))
    ...
    for head in range(1, num_heads + 1):
        head_paths = sorted(
            [path for path in hydra_choices if len(path) == head])
        prefixes = sorted(
            list(set([tuple(head_path[:-1]) for head_path in head_paths])))
        if head == 1:
    ...
    for head in range(num_heads):
        parents_per_head.append(
            sorted([parent for parent in parents if len(parent) == head]))

    def descendant_exists(ancestor, edges_away):
        suff_depth_away = [path for path in nodes if len(
            path) >= len(ancestor) + edges_away]
    ...
    for head_idx, (parents_at_head, head_children) in enumerate(zip(parents_per_head, children_per_head)):
        ...
        for parent, parent_num_children in zip(parents_at_head, head_children):
            ...
            for child_idx in range(parent_num_children):
                ...
                if descendant_exists(child_path, 1):
                    parent_num_expand += 1
    ...
    if sorted_hydra_choices_with_children:
        for head_idx in range(max(sorted_hydra_choices_with_children_per_head.keys())):
            head_choices_with_children = sorted_hydra_choices_with_children_per_head[
                head_idx + 1]
            ancestor_choices = [choice for h, choices in sorted_hydra_choices_with_grandchildren_per_head.items(
            ) for choice in choices if h < head_idx + 1]
            proposal_cross_attn_mask = torch.zeros(
                len(head_choices_with_children), len(ancestor_choices) + 1)
            ...
            for choice_idx, head_choice_with_children in enumerate(head_choices_with_children):
                for ancestor_idx, ancestor_choice in enumerate(ancestor_choices):
                    if head_choice_with_children[:len(ancestor_choice)] == ancestor_choice:
                        proposal_cross_attn_mask[choice_idx,
                                                 ancestor_idx + 1] = 1
            proposal_cross_attn_masks_per_head.append(
                proposal_cross_attn_mask.unsqueeze(0).unsqueeze(0))
    ...
    for head_idx in range(1, num_heads + 1):
        head_choices = [
            choice for choice in hydra_choices if len(choice) == head_idx]
...
def generate_candidates(hydra_logits, logits, beam_sizes, tree_indices, retrieve_indices):
    ...
    for hydra_head, beam_size in enumerate(beam_sizes):
        candidates_hydra_logits.append(torch.topk(
            hydra_logits[hydra_head, 0, -1], beam_size, dim=-1).indices)
    ...
    candidates = torch.cat(
        [candidates_logit, candidates_hydra_logits.view(-1)], dim=-1)

    # Map the combined candidates to the tree indices to get tree candidates.
    tree_candidates = candidates[tree_indices]

    # Extend the tree candidates by appending a zero.
    tree_candidates_ext = torch.cat([tree_candidates, torch.zeros(
        (1), dtype=torch.long, device=tree_candidates.device)], dim=0)
...
def ensemble_tree_decoding(
    ...
    - tuple: Returns hydra logits, regular logits, and other outputs from the model.
    """
    hydra_position_ids = ensemble_model[0].hydra_buffers["hydra_position_ids"]
    ...
    # Use the model to decode the tree candidates.
    # The model is expected to return logits for the Hydra structure, original logits, and possibly other outputs.
    _, outputs, tree_logits, _ = ensemble_model[0](
        tree_candidates,
        output_orig=True,
        past_key_values=past_key_values,
        position_ids=position_ids,
...
def evaluate_posterior(
    ...
    if temperature == 0:
        ...
        accept_length = (torch.minimum(
            candidates_accept_length, max_accepts)).max()
    ...
    if accept_length == 0:
        ...
        best_candidates = torch.where(torch.minimum(
            candidates_accept_length, max_accepts) == accept_length)[0]
